...
const char pathPrefix[10][11] = {"",        "sd:/",    "usb:/",
                                 "dvd:/",   "smb:/",   "carda:/",
                                 "cardb:/", "port2:/", "gcloader:/"};
...
enum { SAVEFOLDER_SAVES = 0, SAVEFOLDER_LENGTH };
...
const FolderDef saveFolder[] = {{SAVEFOLDER_SAVES, "saves"}};

const FolderDef loadFolder[] = {{LOADFOLDER_ROMS, "roms"},
                                {LOADFOLDER_SCREENSHOTS, "screenshots"},
                                {LOADFOLDER_COVERS, "covers"},
                                {LOADFOLDER_ARTWORK, "artwork"},
                                {LOADFOLDER_BORDERS, "borders"}};

enum { FILE_SRAM, FILE_SNAPSHOT, FILE_ROM, FILE_BORDER_PNG };
...
struct SGCSettings {
  ...
  int videomode;  // 0 - automatic, 1 - NTSC (480i), 2 - Progressive (480p), 3 -
                  // PAL (50Hz), 4 - PAL (60Hz)
  int scaling;    // 0 - default, 1 - partial stretch, 2 - stretch to fit, 3 -
...
static inline bool IsWiiUFastCPU(void) {
  return ((*(vu16 *)0xCD8005A0 == 0xCAFE) &&
          ((*(vu32 *)0xCD8005B0 & 0x20) == 0));
}
